<template>
	<div class="card bg-base-100 shadow-xl border-gray-200 border-2">
		<div class="card-body p-4">
			...
			<div class="card-actions justify-between align-center gap-2 flex-nowrap">
				...
				<div class="join">
					...
					>
						<option
							v-for="({ _selected, label }, index) in status"
							:key="index"
							:selected="_selected"
						>
							{{ label }}
...
// 	return status.value.filter((status) => status._selected);
// });

const status = ref<{ _selected: boolean; label: string }[]>([
